...
function config() {
    ...
    $config.endtime_message = "<a id='sms-login-btn' class='span-primary'>درخواست کد جدید</a>";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.body.addEventListener('click', function (e) {
        if (e.target && e.target.id === 'sms-login-btn') {
            e.preventDefault(); // Prevent the default link behavior
            
            // AJAX request to request a new OTP
            $.ajax({
                url: 'http://127.0.0.1:8000/auth/otp-generate/', // Replace with your actual URL
                type: 'POST',
                data: {
                    csrfmiddlewaretoken: $('input[name=csrfmiddlewaretoken]').val()
                },
                success: function(response) {
                    alert("کد جدید با موفقیت ارسال شد."); // Display a success message or update the UI
                    location.reload()
                },
                error: function(xhr, status, error) {
                    console.error('Error:', error);
                    alert("خطایی رخ داده است. لطفا دوباره امتحان کنید."); // Display an error message
                }
            });
        }
    });
